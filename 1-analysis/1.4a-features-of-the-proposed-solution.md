...
### Encryption technique requires Nodes to individually validate blocks.

This approaches means that any block on the chain is not necessarily secure, and Nodes should  assume that it is not secure until they have decrypted the hash using the sender's publicly accessible key and validated it to the hash of the block they are looking at
