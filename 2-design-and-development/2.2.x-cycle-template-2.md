...
<table><thead><tr><th>Test</th><th>Instructions</th><th>What I expect</th><th>What actually happens</th><th data-type="checkbox">Passed?</th></tr></thead><tbody><tr><td>1</td><td>Use the Shell function to echo a message</td><td>The message to be echo'ed from the terminal</td><td>As Expected</td><td>true</td></tr><tr><td>1</td><td>Launch the database.</td><td>The database to launch and no crashes to occur.</td><td>As Expected</td><td>true</td></tr><tr><td>2</td><td>Stop the database.</td><td>The database to stop and no crashes to occur.</td><td>As Expected</td><td>true</td></tr><tr><td>3</td><td>Launch and then connect to the database.</td><td>The database to launch then accept the program's connection without any crashes.</td><td>As Expected</td><td>true</td></tr><tr><td>4</td><td>Get the last 5 messages from the database.</td><td>An array of messages to be returned in the expected type.</td><td>As Expected</td><td>true</td></tr><tr><td>5</td><td>Create a node reference in the database.</td><td>The reference to be created in the database.</td><td>As Expected</td><td>true</td></tr><tr><td>6</td><td>Create a node reference and then check if the software is aware of that node.</td><td>The "aware_of" check should return true for the reference test that already exists.</td><td>As Expected</td><td>true</td></tr><tr><td>7</td><td>Collect all references from the database.</td><td>Should return an array of node references in the expected type.</td><td>As Expected</td><td>true</td></tr></tbody></table>
{% endtab %}
